...
def student_exam_view(request):
    ...
    for t in courses: 
        ...
        if(temp_date>t.exam_date):
            ...
            if(temp_date==t.exam_date):
                if(temp_time>t.exam_end_time):
                    status.append("Past")
                    t.status="Past"
                    t.save()
                elif(temp_time<t.exam_time):
                    status.append("Upcoming")
                    t.status="Upcoming"
                    t.save()
                elif(temp_time>t.exam_time and temp_time<t.exam_end_time):
                    status.append("Live")
                    t.status = "Live"
                    t.save()
    count = len(status)   
...
def calculate_marks_view(request):
    if request.COOKIES.get('course_id') is not None:
        ...
        if(QMODEL.Result.objects.filter(student_id=student.id,exam_id=course_id).exists()==False):
